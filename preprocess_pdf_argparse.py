...
def main():
    ...
    for audio_file, pdf_file in zip(audio_file_list, pdf_file_list):
        try:
            res = get_split(pdf_file_path + pdf_file)    
            # chunk_path, durations = chunk_audio(audio_file_path+audio_file, res, output_file_path+audio_file[:-4])
            chunk_path, durations = chunk_audio(audio_file_path+audio_file, res, output_file_path+audio_file)
